...
function buildNet(cnn, params, style_images_caffe)
    ...
    if next_content_i <= #content_layers or next_style_i <= #style_layers or next_temporal_i <= #temporal_layers then
      ...
      if name == style_layers[next_style_i] then
        ...
        for i = 1, #style_images_caffe do
          ...
          --style_mask_path = '/home/panda/Desktop/style/RealTime/Real-Time-Video-Transfer-master/video_playingpiano/style_mask.jpg'
          --local style_mask = image.load(style_mask_path, 3)
          ...
          --print(style_mask:size(1))
          --print(style_mask:size(2))
          --
          --style_mask = image.scale(style_mask, target_features:size(3), target_features:size(2), 'bilinear')
          --style_mask = torch.CudaTensor(style_mask:cuda())
          ----style_mask = style_mask:view(1, 240, 320):expandAs(target_features)
          --
          --print(style_mask.iscuda)
          --print(style_mask:size(1))
          --print(style_mask:size(2))
          ----print(style_mask:size(3))
          ----style_mask = style_mask:view(1, 240, 320)
          --print(target_features.iscuda)
          --print(target_features:size(2))
          --print(target_features:size(3))
          --print('......')



          --for ii=1, target_features:size(1) do
          --  for jj=1, style_mask:size(1) do
          --    for kk=1, style_mask:size(2) do
          --      target_features[ii][jj][kk] = target_features[ii][jj][kk]*style_mask[jj][kk]
          --    end
          --  end
          --  print(ii)
          --end
  ...
  if params.sim_weight > 0 then
    local sim_mod = nn.SimLoss(params.sim_weight):float()
    sim_mod = MaybePutOnGPU(sim_mod, params)
    net:add(sim_mod)
    current_layer_index = current_layer_index + 1
  end
  print(net)
...
function TVLoss:updateGradInput(input, gradOutput)
  ...
  --print(self.x_diff:size(1), self.x_diff:size(2), self.x_diff:size(3))
  ...
  --print(self.gradInput:size(1), self.gradInput:size(2), self.gradInput:size(3))
  self.gradInput[{{}, {1, -2}, {2, -1}}]:add(-1, self.x_diff)
  self.gradInput[{{}, {2, -1}, {1, -2}}]:add(-1, self.y_diff)
  self.gradInput:mul(self.strength)
  self.gradInput:add(gradOutput)
  --print(self.gradInput:size(1), self.gradInput:size(2), self.gradInput:size(3))
  --print(self.gradInput[1][1][1])
  return self.gradInput
end

local SimLoss, parent = torch.class('nn.SimLoss', 'nn.Module')

function SimLoss:__init(strength)
  parent.__init(self)
  self.strength = strength
  self.diff1 = torch.DoubleTensor()
  self.diff2 = torch.DoubleTensor()
  self.diff3 = torch.DoubleTensor()
  self.num = torch.DoubleTensor()
end

function SimLoss:updateOutput(input)
  self.output = input
  return self.output
end

function SimLoss:updateGradInput(input, gradOutput)
  print("sim")
  self.gradInput:resizeAs(input):zero()
  local C, H, W = input:size(1), input:size(2), input:size(3)
  self.num:resize(1, H, W):zero()
  self.diff1:resize(1, H, W)
  self.diff2:resize(1, H, W)
  self.diff3:resize(1, H, W)
  self.diff1:copy(input[1] - input[2])
  self.diff2:copy(input[2] - input[3])
  self.diff3:copy(input[1] - input[3])
  self.diff1 = self.diff1:pow(2)
  print(self.diff1[1][1][1])
  self.diff2 = self.diff2:pow(2)
  print(self.diff2[1][1][1])
  self.diff3 = self.diff3:pow(2)
  print(self.diff3[1][1][1])
  self.num:add(self.diff1):add(self.diff2):add(self.diff3)
  print(self.num[1][1][1])
  --self.gradInput = self.num:norm()
  --print(self.gradInput)
  self.gradInput[1] = self.num
  self.gradInput[2] = self.num
  self.gradInput[3] = self.num
...
function getStyleImages(params)
  ...
  for _, img_path in ipairs(style_image_list) do
    ...
    --print(firstContentImg:size(2))
    --print(firstContentImg:size(3))
    --print(img:size(2))
    --print(img:size(3))
    --print(params.style_scale)
    -- Scale the style image so that it's area equals the area of the content image multiplied by the style scale.
    ...
    img = image.scale(img, 320, 240, 'bilinear')
