...
def targeted_video_attack(vid_model, vid, target_vid, target_class,
    ...
    while num_iter < max_iter:
        ...
        if target_class != top_idx[0][0]:
            logging.info('not right'.format(num_iter))
            return False, num_iter, adv_vid

        l, g = apply_NES(vid_model, adv_vid, sample_per_draw, sigma,
                                        target_class, rank_transform, sub_num_sample, untargeted=False)
        if l is None and g is None:
            logging.info('nes sim fails, try again....')
            continue

        grad_direction = torch.sign(g)
        ...
        while True:
            ...
            # PGD
            proposed_adv_vid -= cur_lr * grad_direction
...
def untargeted_video_attack(vid_model, vid, ori_class,
    ...
    while num_iter < max_iter:
        ...
        l, g = apply_NES(vid_model, adv_vid, sample_per_draw, sigma,
                                        ori_class, rank_transform, sub_num_sample, untargeted=True)
