...
function optim.lbfgs(opfunc, x, config, state)
   ...
   local tolFunRelative = tonumber(config.tolFunRelative) or 0
   local tolFunRelativeInterval = tonumber(config.tolFunRelativeInterval) or 100
   ...
   while nIter < maxIter do
      ...
      if nIter % tolFunRelativeInterval == 0 then
        if f_past ~= nil and (abs(f-f_past) / f_past) < tolFunRelative then
          verbose('relative change in function value is less than tolFunRelative')
          break
        end
        f_past = f
      end
